...
recognised by their titles: broadcast marks ([字], [再], 【HV】, ＜再＞ ...) are dropped, the text is cut at the
first episode marker (第３話, ＃１２, （３５）, 後編, and for sport 第３戦, １０日目, 決勝, 千秋楽 ...) or, failing
that, at the first subtitle separator (full-width space, ▽, 「…」), and the remainder is normalised into a key.
"""
...
_PUA = re.compile(r"[-]")
_MARKS = re.compile(
    r"\[(?:字|解|再|新|終|デ|二|多|SS|映|生|手|4K|HDR|5\.1|7\.1|22\.2|3D|2K|8K)\]"
    r"|[［【＜（](?:字|解|再|新|終|初|デ|二|多|双|映|生|手|吹|声|無料|無|料|鍵|天|交|販|演|他|前|後|HV|SD|SS|PPV|MV|W|[SBNPＳＢＮＰ])[］】＞）]"
)
_EPISODE = re.compile(
    r"(?:"
    r"第\s*[0-9０-９〇一二三四五六七八九十百]+\s*(?:話|回|夜|章|部|集|弾|幕|箱|日目|日|週|戦)"
    r"|[#＃♯]\s*[0-9０-９]+"
    r"|[（(]\s*[0-9０-９]+\s*[)）]"
    r"|(?<![0-9０-９])[0-9０-９]{1,3}\s*(?:話|回戦|回目?|日目)(?![0-9０-９])"
    r"|(?<![a-zA-Z])(?:ep|episode|season)(?![a-zA-Z])|シーズン|前[編篇]|後[編篇]|総集[編篇]|最終[回話戦節日]"
    r"|初戦|初回|初日(?!の出)|開幕戦|千秋楽|準々決勝|準決勝|決勝"
    r")",
    re.IGNORECASE,
)
_SEPARATORS = re.compile(r"[　▽▼▲△◆◇■□●○★☆※…：／｜～〜]")
# ▽ and ▼ introduce a subtitle; when the episode marker comes later, what sits between them is not the name
_TOPICS = re.compile(r"[▽▼]")
