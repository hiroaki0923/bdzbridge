...
def test_port_vectors_round_trip():
    ...
    assert epg["services"][1]["programs"][0]["reference"] and epg["services"][0]["programs"][0]["title"] == "ＮＨＫニュース　おはよう日本[字]"
    series = json.loads(out[portkit.OUT / "series.json"])
    by_title = {c["title"]: c for c in series["titles"]}
    assert by_title["日曜劇場「VIVANT」 第1話"]["series_key"] == by_title["日曜劇場「ＶＩＶＡＮＴ」第１８話　前半戦完結　乃木＆ノコル"]["series_key"]
