...
def test_channels_and_programs(client):
    ...
    assert one["title"] == "ＮＨＫニュース　おはよう日本"
    q = client.get("/api/v1/programs", headers=H, params={"q": "あさイチ"}).json()
...
def test_reference_programs_resolve_to_parent(client):
    ...
    assert row.is_reference and row.title == "ＮＨＫニュース　おはよう日本"
...
def test_search_ignores_width_and_case(client):
    for term in ["vivant", "ＶＩＶＡＮＴ", "Vivant"]:
...
def test_rules_reserve_matching_programs_once(client):
    ...
    r = c.post("/api/v1/rules", headers=H, json={"query": "vivant"}, params={"run": "true"})
    ...
    assert len(res) == 1 and res[0]["title"] == "日曜劇場「ＶＩＶＡＮＴ」" and res[0]["quality"] == "LSR"
...
def test_rules_report_conflicts_without_reserving(client):
    ...
    c.post("/api/v1/rules", headers=H, json={"query": "あさイチ", "quality": "DR"})
    run = c.post("/api/v1/rules/run", headers=H).json()
...
def test_hidden_channels_are_skipped_by_rules(client):
    ...
    rid = c.post("/api/v1/rules", headers=H, json={"query": "vivant"}).json()["id"]
