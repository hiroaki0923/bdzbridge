...
from recbridge.notify import Notifier
...
def test_mail_goes_through_smtp(monkeypatch):
    monkeypatch.setattr("recbridge.notify.smtplib.SMTP", FakeSMTP)
    ...
    assert asyncio.run(n.send("[recbridge] 自動予約 1 件", "本文\n")) == ["email"]
    ...
    assert msg["Subject"] == "[recbridge] 自動予約 1 件" and msg.get_content().startswith("本文")


@pytest.mark.parametrize("port,starttls", [(465, False), (25, False)])
def test_mail_transport_variants(monkeypatch, port, starttls):
    monkeypatch.setattr("recbridge.notify.smtplib.SMTP", FakeSMTP)
    monkeypatch.setattr("recbridge.notify.smtplib.SMTP_SSL", FakeSMTP)
    FakeSMTP.instances.clear()
