...
できた条件を本体の画面で見ると「対象チャンネル: すべてのチャンネル」でした。**黙って捨てられています。**
試した書式は 7 通りで、結果はすべて同じです。

- `011` — 本体の画面に出る 3 桁番号（`011` は NHK総合1）
- `0x0400` / `1024` — service_id を 16 進で / 10 進で
- `11` — 3 桁番号から先頭の 0 を落としたもの
