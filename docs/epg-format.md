...
レコーダーが番組表ファイルを作り直すのは夜中（ロゴファイルのヘッダーでは 00:51）なので、**再起動した日は
その時刻まで番組表を取得できない**と考えるのが安全です。クライアントは 500 を「異常」ではなく
...
作り直しを経てもゼロのままでした。ロゴは放送波の CDT で流れてくるものなので、受信して溜まるのを待つか、
本体でチャンネルの再スキャンをするのが確実です。アプリ側としては、ロゴがない局は名前だけで表示する作りに
しておく必要があります。
