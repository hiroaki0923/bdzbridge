...
| Wake-on-LAN | `NWConnection` の UDP。宛先は 3 つ（`255.255.255.255`、レコーダーのサブネットのブロードキャスト、**レコーダーのアドレス宛のユニキャスト**）。VPN 越しに届くのはユニキャストだけ |
...
- **Wake-on-LAN**: ブロードキャスト UDP はどちらの OS でも送れます。MAC は端末から ARP を読めませんが、
...
| 探索と Wake-on-LAN | 完了。/24 の TCP スキャン、MAC の自動記録、起動待ちの表示 |
