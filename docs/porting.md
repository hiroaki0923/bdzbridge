...
- **Wake-on-LAN は同じサブネットにいるときだけ確実です。** `255.255.255.255` はルーターを越えず、VPN 越しの端末は
  ...
  ブロードキャストを一切使わず、同じセグメントからアドレス宛にポート 9 と 7 へ送って 6 秒で応答）。ただし送り手が
  その MAC を解決できることが前提なので、ルーター越しに届くのはゲートウェイがその MAC を覚えている間だけと
  考えるべきです（落ちたレコーダーが ARP に答えるかは未確認）。キャッシュが切れた後は LAN の内側にいる何かに
  送ってもらうしかありません。自宅にサーバーを置いているなら `POST /api/v1/recorder/wake` がその役目を果たします。
