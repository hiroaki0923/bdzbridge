...
- **Wake-on-LAN は同じサブネットにいるときだけ確実です。** `255.255.255.255` はルーターを越えず、VPN 越しの端末は
  自分のインターフェースから自宅のサブネットを知ることができません。レコーダーのアドレスから
  レコーダーのアドレスからそのサブネットのブロードキャストを組み立てて送る手はありますが、ゲートウェイがディレクテッドブロードキャストを転送する
  設定でなければ届きません（UniFi は既定で転送しません）。レコーダーのアドレスへ直接送る手は、**ルーターが
  そのアドレスの MAC をまだ覚えている間だけ**有効です。落ちたレコーダーは ARP に答えないので、キャッシュが
  切れた後は LAN の内側にいる何かに送ってもらうしかありません。自宅にサーバーを置いているなら
  `POST /api/v1/recorder/wake` がその役目を果たします。
...
- DLNA ツリーにはシリーズ ID がない。「まとめ」はタイトル文字列から `series.py` の規則で作る。
...
0. **アプリの現状**: `app/RecorderApp` に SwiftUI の 5 画面（番組表・検索・予約・録画・設定）があり、実機の
