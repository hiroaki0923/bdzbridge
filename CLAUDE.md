...
- `app/` — the iOS app (Swift/SwiftUI). `app/RecorderKit` is the recorder-facing Swift package: protocols, decoders, the HTTP client and the guide cache, no UI. Tested headlessly against `docs/port/` with a stubbed transport; `RECORDER_HOST=<ip> swift test --filter LiveRecorderTests` reads a real recorder. `app/BDBridge` is the app; its Xcode project and Info.plist are generated from `app/project.yml` by XcodeGen and committed, so that a clone builds and Xcode Cloud can find a project — regenerate and commit them after editing `project.yml`. `app/BDBridgeUITests` holds only the App Store screenshots, taken by `app/scripts/screenshots/capture.sh` against the invented recorder and guide in `BDBridge/DemoData.swift` (`#if DEBUG`, `-demoData 1`), so that nothing real is ever in a picture; the listing text is `docs/app-store-description.md`. See `app/README.md`.
