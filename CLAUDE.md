...
- `bdzbridge/state.py` — `Bridge`: settings, the selected recorder or None, store, notifier, the cached full title list, and the bodies of the bulk operations (delete / protect / duplicate scan). `bdzbridge/jobs.py` runs those as cancellable background jobs: POST starts one and answers 202 with the job, `GET /jobs/{id}` polls it, `POST /jobs/{id}/cancel` stops it after the current item (`Job.step()` is the checkpoint). The web app follows jobs with `src/jobs.js`. Recorder selection order: `BDZBRIDGE_RECORDER_HOST` → host saved in SQLite `meta` (re-discovered by UDN if it moved) → unconfigured (only discovery/select and cached EPG work). Tests inject a fake recorder through `create_app(settings, bridge)`; the fakes and the `client` fixture live in `tests/conftest.py`, API tests are split by area (`tests/test_api_*.py`).
