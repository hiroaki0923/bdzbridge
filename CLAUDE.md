...
```
uv sync --group dev
uv run pytest -q                       # no recorder needed; the real-file test skips unless RECBRIDGE_TEST_EPG_FILE is set
uv run pytest -q tests/test_xsrs.py::test_create_elements_match_official_app
uv run ruff check recbridge tests
cp .env.example .env                   # set RECBRIDGE_API_TOKEN; recorder host optional
uv run python -m recbridge discover    # list recorders on the LAN
uv run python -m recbridge             # serve; OpenAPI at /docs
```

Web (inside `web/`): `npm install`, `npm run build` (writes `web/dist`; restart the server to pick it up), `npm run dev`.

Every API call needs `Authorization: Bearer <RECBRIDGE_API_TOKEN>`.
