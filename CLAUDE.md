...
- `app/` — the iOS app (Swift/SwiftUI). `app/RecorderKit` is the recorder-facing Swift package: protocols, decoders, the HTTP client and the guide cache, no UI. Tested headlessly against `docs/port/` with a stubbed transport; `RECORDER_HOST=<ip> swift test --filter LiveRecorderTests` reads a real recorder. `app/BDBridge` is the app; its Xcode project and Info.plist are generated from `app/project.yml` by XcodeGen and committed, so that a clone builds and Xcode Cloud can find a project — regenerate and commit them after editing `project.yml`. `BDBridge/DemoData.swift` is an invented recorder (`DemoRecorder` answers the real XML and remembers what is done to it) and an invented guide, offered in the tutorial to anyone without a recorder and used for the App Store screenshots so that nothing real is ever in a picture; it writes its guide to `guide-demo.sqlite3` and ending it deletes that file. `app/BDBridgeTests` holds `AppModel`'s unit tests: they run inside the app, which leaves out its own start while it hosts them (`BDBridgeApp.hostingUnitTests`), and each builds a model on `Surroundings` of its own (`BDBridge/Surroundings.swift`: a throwaway defaults suite and folder, the demo's `DemoRecorder` or a silent recorder as the transport, a network it changes itself, nothing put on the LAN). `app/BDBridgeUITests` holds the screenshots (`app/scripts/screenshots/capture.sh`) and the demo's UI tests. Xcode Cloud runs `ci_scripts/ci_post_clone.sh` (regenerates the project) and `ci_scripts/ci_pre_xcodebuild.sh` (RecorderKit's `swift test` before an archive; a failure fails the build). See `app/README.md`.
- `docs/` — protocol references: `xsrs-api.md` (reservations), `epg-format.md` (EPG files), `upnp/` (the recorder's UPnP descriptions). Read them before touching recorder code. `porting.md` is what the iOS app was built from and what a port to another platform would need: the recorder's behaviour checklist, the device-side design notes and where the app now stands, and `port/` holds generated conformance vectors (`bdzbridge/tools/portkit.py`; a test fails while they are stale, the pre-commit hook regenerates them).
...
```
xcodegen generate                      # rewrites BDBridge.xcodeproj and BDBridge/Info.plist; commit them
xcodebuild -project BDBridge.xcodeproj -scheme BDBridge -destination 'id=<simulator udid>' build
xcodebuild test -project BDBridge.xcodeproj -scheme BDBridge -destination 'id=<simulator udid>' -only-testing:BDBridgeTests
xcodebuild test -project BDBridge.xcodeproj -scheme BDBridge -destination 'id=<simulator udid>' -only-testing:BDBridgeUITests/DemoModeTests
```
