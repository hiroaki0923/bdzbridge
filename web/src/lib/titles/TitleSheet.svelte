...
<div class="sheet" style="z-index: 23">
  ...
  <h2>{#if t.protected}<span class="lock" title="保護中">🔒</span>{/if}{t.title}</h2>
  ...
  <button class="btn ghost" disabled={flagBusy} onclick={() => setProtected(!t.protected)}>{t.protected ? '保護を解除する' : '保護する（自動削除させない）'}</button>
  <button class="btn ghost danger-text" disabled={t.protected} onclick={() => (confirm = true)}>{t.protected ? '保護中は削除できません' : 'この録画を削除'}</button>
