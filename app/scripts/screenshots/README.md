...
中身は `app/BDBridge/DemoData.swift`。偽レコーダー（`DemoRecorder`）と偽番組表からできて
いて、局名・番組名・録画タイトル・キーワード・IP・MAC はすべて架空。**これはチュートリアルが
出す「サンプルデータで試す」と同じもの**で、製品ビルドにも入っている（レコーダーを持っていない
人と審査担当者のため）。`-demoData 1` は、それをコマンドラインから入れるための引数。

そのため、**撮れる画面の上には「サンプルデータを表示しています」の帯が写る。** 中身が架空で
あることを言っている帯なので、そのまま出して構わない。出したくなければ帯を隠す引数を足す。

掲載文（説明文・キーワード・審査メモ）は `notes/app-store-description.md`（コミットしない）。
...
必要がない（引数のドメインは保存された設定より優先される）。アプリが起動をまたいで覚えている
放送の種類・並び順・既定の録画モードも、`-guideBroadcasting td` / `-reservationSort time` /
`-recordingsSort newest` / `-defaultQuality LSR` で固定している。
...
- **重なる予約の確認を待ってから撮る。** 番組シートはレコーダーに時間の重なる予約を問い合わせるので、
  「時間が重なる予約はありません」が出るのを待たないと空の行が写る
- 画面が出た直後は List のアニメーションが途中なので、1 秒ほど置いてから撮っている
- **デモは状態を持つ。** 予約を作れば一覧に出るし、消せば消える（`DemoRecorder` が覚えている）。
  シミュレータに残るので、撮り直しの前に前回の操作が残っていないか気にするなら、シミュレータを
  消すか、設定からサンプルデータを終了する
